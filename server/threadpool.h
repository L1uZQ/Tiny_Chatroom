...
struct roominfo{
    ...
    int fd;
    int epfd;
    static unordered_map<int,bool>sign;
    // static std::unordered_map<int, bool> externalSign;
    // static void associateSign(const std::unordered_map<int, bool>& externalSign); // 关联函数声明 
};
...
void process(roominfo& work){
    ...
    // cout<<"***work->hd->account***"<<account<<endl;
    // cout<<"***work->hd->dataType***"<<dataType<<endl;
    // cout<<"***work->hd->dataLength***"<<dataLength<<endl;
    DataProcesser dp;
    switch (protocolId) {
        case LOGIN: {
